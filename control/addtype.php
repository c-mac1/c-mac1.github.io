...
<link rel="stylesheet" href ="../ui.css">
    ...
    <ul class="navbar-nav mr-auto">
      ...
      <li class="nav-item dropdown">
        ...
        <li class="nav-item right">
        <a class='nav-link' href='../admin/ogout.php'>Sign Out <i class='fas fa-sign-out-alt''></i></a>
        </li>
	...
	<div class="container">
		<div class ="row">
  		...
  		<form method="POST" action = "addtype.php">
    		...
    		<label for="exampleInputEmail1">Insert a new Product Type</label>
    		<input type="text" class="form-control" id="type" aria-describedby="type" placeholder="Enter Type" name="typepost">
  			</div>
    		
    		<button type="submit" class="btn btn-primary" name="submitnt">Submit</button>
  			
  			</div>
  
		</form>
		<?php
		//only happens when submit is clicked 
		
		if (isset($_POST['submitnt'])){
		
		$type =$conn->real_escape_string($_POST['typepost']);
		
		//add security
		
		$inserttype = "INSERT INTO WD_Product_Type (Type) VALUES ('$type')";
		
		$insertresult = $conn->query($inserttype);
		
		if(!$insertresult){
			echo $conn->error;
		} 

		echo "<p>New product type $type added</p> ";
