...
//as was posted throughurl use GET. filter is a key
$getprodtype = $conn->real_escape_string($_GET['filter']);



//the $getproduct var hold the id of the product eg 1 = jersey 
$read = "SELECT WD_Products.description, WD_Products.id, WD_USERS.email, WD_Products.published, WD_Products.Information, WD_Products.price, WD_Product_Type.Type, WD_Products.path
			FROM WD_USERS
            INNER JOIN WD_Products
            ON WD_USERS.id = WD_Products.userid
			INNER JOIN WD_Product_Type 
			ON 
			WD_Products.Product_type_id = WD_Product_Type.id WHERE WD_Products.Product_type_id = '$getprodtype'";
