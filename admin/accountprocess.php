...
session_start();
	...
	$random = rand(10,1000);
	$user_data =$conn->real_escape_string($_POST['editname']);
	
	$id = $conn->real_escape_string($_POST['myeditacid']);
	
	$edit_pw = $conn->real_escape_string($_POST['editpass']);
	$edit_email = $conn->real_escape_string($_POST['editemail']);
	$edit_phone = $conn->real_escape_string($_POST['editphone']);
	$edit_school = $conn->real_escape_string($_POST['edit_school']);
	$edit_smedia = $conn->real_escape_string($_POST['editsmedia']);
	
	$profileimg_update =$_FILES['editprofile']['name'];
	$profileimg_update = $random.$profileimg_update;
	$edittmpimg_update=$_FILES['editprofile']['tmp_name'];


	move_uploaded_file($edittmpimg_update, "../reglog/users/".$profileimg_update);
	
	$update = "UPDATE WD_USERS SET user = '$user_data', pw ='$edit_pw', socialmedia = '$edit_smedia', email='$edit_email', phone = '$edit_phone', profileimg = '$profileimg_update', school ='$edit_school'  WHERE id = '$id' ";
...
<link rel="stylesheet" href ="../ui.css">
  ...
  <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
	...
	<div class="container">
		...
		<div class='jumbotron'>
  			<h1 class='display-4'>You have updated your account details! <i class='fas fa-edit'></i></h1>
  			<hr class='my-4'>
